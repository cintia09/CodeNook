...
for dir in ~/.copilot/skills/agent-*/; do
...
HOOK_COUNT=$(ls ~/.copilot/hooks/*.sh 2>/dev/null | wc -l | tr -d ' ')
...
for hook in ~/.copilot/hooks/*.sh; do
  [ -x "$hook" ] || { echo "❌ Not executable: $hook"; FAIL=$((FAIL+1)); }
done

# Check hooks.json
[ -f ~/.copilot/hooks/hooks.json ] && echo "✅ hooks.json exists" && PASS=$((PASS+1)) || echo "❌ Missing hooks.json" && FAIL=$((FAIL+1))
