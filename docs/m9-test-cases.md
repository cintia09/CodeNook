...
>
> 关联文档：
> - `docs/memory-and-extraction.md`（设计 / 接口 / 数据模型）
> - `docs/architecture.md` §13（Memory Layer 决策 #53–#59）
> - `docs/implementation.md` §M9（per-milestone 文件 / 测试 / 工时拆解）
...
**里程碑目标摘要**：见 `implementation.md` §M9.1。建立 memory 骨架与
...
**里程碑目标摘要**：见 `implementation.md` §M9.2。把抽取调度接入
...
**里程碑目标摘要**：见 `implementation.md` §M9.3。第一个抽取器，承载
...
**里程碑目标摘要**：见 `implementation.md` §M9.4。检测重复脚本/CLI
...
**里程碑目标摘要**：见 `implementation.md` §M9.5。识别 `task-config-set`
...
**里程碑目标摘要**：见 `implementation.md` §M9.6。router-agent 看见
...
**里程碑目标摘要**：见 `implementation.md` §M9.7。codify「plugins
...
**里程碑目标摘要**：见 `implementation.md` §M9.8。完整链路：用户对话
...
- **类型**: unit
- **前置条件**: 仓库根
- **步骤**: `[ -f docs/memory-and-extraction.md ] && grep -q '§13' docs/architecture.md`
- **期望**: 两条均通过
...
6. **NFR 覆盖**：所有 8 个 NFR 在 §0.6 矩阵中均 ≥ 1 case；review 时复跑
   `grep -c '^| NFR-' docs/m9-test-cases.md` 应 = 8。
