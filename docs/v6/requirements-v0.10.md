...
- **描述**：仓库根 `install.sh` 安装全局 skill 入口（`codenook-init`）到 `~/.copilot/skills/` 与 `~/.claude/skills/`；工作区骨架由 `skills/codenook-core/skills/builtin/init/init.sh` 直接执行（参见 FR-INIT-1）。
- **验收标准**：
  - `bash install.sh --install` exit 0 且 `--check` 显示两端 skill 已就绪；
  - `bash skills/codenook-core/skills/builtin/init/init.sh <ws>` 在 `<ws>` 生成 v0.10 骨架（FR-INIT-1）。
- **源**：`install.sh`；`sync-skills.sh`；`skills/codenook-core/skills/builtin/init/init.sh`。
- **状态**：✅
- **注**：v0.10 验收阶段（FAT-Phase3）将 spec 由"`./install.sh <ws>` 复制 core 到 workspace"修订为以上分工，与代码实际形态对齐（SPEC-PATCH applied 2026-04, A2-NEW）。
...
- **验收标准**：
  - max_iterations > 1 时 dual_mode 必须存在（preflight 检查）；
...
- **验收标准**：
  ...
  - 同一 (task, phase, after_phase) 组合 24h 内幂等（不重复抽取）。
...
- **验收标准**：
  ...
  - 死进程 stale-lock（pid 不存在或 started_at >300s）可恢复；
  - 不可解析 payload → 永不 unlink（保守）。
...
- **验收标准**：
  - 默认 max_depth=10；超出 emit `chain_walk_truncated` audit；
...
- **验收标准**：
  ...
  - 含 EN+ZH 共 ~50 stopwords + <2 字符 token 过滤；
...
- **验收标准**：
  - 重复同 key 24h 内 → 仅返回 `skipped` 列表；
  - extractor 通过 nohup 分离派发；
...
- **描述**：dispatch payload 写日志前去除 9 类常见密钥（OpenAI/Anthropic/AWS/GitHub PAT/PEM 等）。
- **验收标准**：
  - 模式：`sk-`, `sk-proj-`, `sk-ant-`, `sk-ant-api03-`, `AKIA`, `ghp_`, `gho_`, `github_pat_`, PEM 头；
  - 命中即替换为 `[REDACTED]`；
...
- **验收标准**：
  ...
  - 静态：`python plugin_readonly.py --target <dir>` 扫到 open(...,'w') / write_* / shutil.copy* 命中模式 → exit 1；
  - 默认排除 test fixture。
...
| Gate | Skill | 关键规则 | 源 |
|------|-------|---------|----|
| G01 format | plugin-format | plugin.yaml 存在；symlink 不得越界 | `plugin-format/format-check.sh` |
...
| G05 signature | plugin-signature | 默认可选；CODENOOK_REQUIRE_SIG=1 强制；sha256(plugin.yaml) 比对 | `plugin-signature/signature-check.sh` |
...
| G11 path-normalize | plugin-path-normalize | 禁所有 symlink；YAML 不得含绝对/`~`/`..` | `plugin-path-normalize/path-normalize.sh` |
...
- **NFR-REL-4**：extractor 幂等键 = SHA256(task|phase|reason)；24h 内不重抽。
...
- **L-9**：chain depth 默认 10；超长链路被截断（emit `chain_walk_truncated`）但无人工接管入口。
...
- `docs/v6/architecture-v6.md`
- `docs/v6/implementation-v6.md`
- `docs/v6/router-agent-v6.md`
- `docs/v6/memory-and-extraction-v6.md`
- `docs/v6/task-chains-v6.md`
- `docs/v6/test-plan-v6.md`
- `docs/v6/m9-test-cases.md`
- `docs/v6/m10-test-cases.md`
...
1. **dual_mode 默认值**：spec 多处隐含默认 `serial`，但 preflight 仅在 `total_iterations > 1` 时检查 dual_mode 存在；当 max_iterations=1 时缺字段也通过 → 文档应明确"可选 + 缺省视为 serial"。源：`builtin/preflight/_preflight.py`。
2. **chain max_depth 默认 10**：spec §4.1 提"默认 10"，task_chain.py 实现允许 `max_depth=None` 走另一分支（无截断），与 spec 的"必有截断"叙述存在边界差异。源：`_lib/task_chain.py:417`。
3. **plugin.yaml.sig 比较语义**：实现允许"first non-blank token"宽松对比（即文件内可包含注释/换行），spec 未说明此宽松。源：`plugin-signature/_signature_check.py`。
4. **extractor 24h 幂等窗口**：代码用 SHA-256 key 但未显式过期；当前理解的"24h 内幂等"实际取决于 audit 文件的轮转策略（未实现轮转）→ 严格说"永久幂等"。源：`extractor-batch/extractor-batch.sh`。
5. **secret patterns 数量**：spec 提"多类 secret 模式"，secret_scan.py 实测为 9 条；与某些文档处提到的"10 条"不符。源：`_lib/secret_scan.py:21`。
6. **session-resume M1 兼容键**：实现保留了一组"M1-compat keys"（active_task / phase / iteration / summary / hitl_pending / next_suggested_action / last_action_ts / total_iterations）—— v6 spec 已声明 greenfield，理论上可删除，但代码仍保留并占预算。
7. **router-agent --confirm 的退出码 4**：spec 中标记为"validation failure"，但实现把 draft yaml 解析错误也归在 4；与文档枚举 0/2/3/4 描述粒度不一致。
8. **plugin-format 与 plugin-path-normalize 的 symlink 策略不同**：G01 允许内部相对 symlink、仅禁越界；G11 禁止所有 symlink。spec 在两处都用"symlink 受限"模糊描述，未明确双闸门差异。

### A.2 代码已实现但 spec 未文档化

1. **plugin_readonly 静态 CLI 模式**：spec 仅描述运行时 guard，未提"独立扫描器" + 默认排除 test-fixture。源：`_lib/plugin_readonly.py:263`。
2. **parent_suggester 内置 EN+ZH stopwords 表**（~70 词）：spec §5 写"分词 + jaccard"但未公开停用词清单，可作为 plugin 调参依据。
3. **task_lock stale 判定阈值 300s + 不可解析 payload 永不 unlink**：spec 提"恢复死锁"但未规定 300s 阈值与"保守不删"策略。
4. **memory_gc 对 promoted 条目的跳过逻辑**：spec 仅提"caps"，未提 `promoted=true` 的条目永不被淘汰。
5. **config-resolve 兜底链 strong→balanced→cheap→opus-4.7 硬编码**：spec 提 fallback 但未规定具体顺序与最末兜底模型 ID。
6. **dispatch-audit redaction 名单**：实现含 9 类常见密钥模式，spec 仅提"redact secrets"未列具体模式表。
7. **router-agent 在 --user-turn-file 模式下的 stdin 行为**：实现支持 `-` 读 stdin，文档未提。
8. **distiller 表达式 sandbox 禁用 `__` 与 `import`**：spec 提"安全表达式"未给禁用清单。
9. **extractor-batch 通过 `nohup` 分离派发**：spec 仅提"detached"未明确机制，影响 ops 排查。
10. **plugin_manifest_index DEFAULT_PRIORITY = 100**：spec 提 priority 平分但未给默认。

### A.3 待验收阶段确认事项

- A.1 项目应在下一次 spec patch 中明确化；
- A.2 项目应判定是否补入 spec 或保留为内部细节；
- 需要评估是否新增 NFR-COMPAT-4 明确"Windows = unsupported"（避免误装）。
