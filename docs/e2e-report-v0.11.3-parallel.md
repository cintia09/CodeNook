...
## v0.13.1 follow-up — round-2 fixes forward-ported (delivered 2026-04-20)

The 9 round-2 fixes originally shipped on `release/v0.11.4` are now on
`main` on top of v0.12 (native Windows port) and v0.13 (self-contained
workspace kernel). Tag `v0.13.1`. Bats 914, pytest 32 — all green.

| ID         | Severity | Resolution                                                                                       | Commit (cherry-pick of v0.11.4) |
|------------|----------|--------------------------------------------------------------------------------------------------|---------------------------------|
| E2E-P-001  | HIGH     | install.sh stamps `state.json.kernel_version` from VERSION; post-install assertion verifies      | `41b62cc`                       |
| E2E-P-002  | HIGH     | `task new` returns entry-question JSON + exit 2 when `--dual-mode` missing                       | `a3d9558`                       |
| E2E-P-003  | HIGH     | extractor falls back to `summary` + body when no `extract:` block; integration test added        | `4b60dce`                       |
| E2E-P-004  | MEDIUM   | claude_md_linter per-token inside-marker allowlist for kernel references                         | `2e5b7aa`                       |
| E2E-P-005  | MEDIUM   | `--target-dir` flag (default `src/`); tick entry-question on missing; `task set --field` subcmd  | `8ba110e`                       |
| E2E-P-006  | MEDIUM   | `state.example.md` moved to `.codenook/schemas/`; bootloader + installer aligned                 | `b02309c`                       |
| E2E-P-007  | LOW      | dispatch-audit + hitl-adapter tee per-task `audit.jsonl`                                         | `88a41c4`                       |
| E2E-P-008  | LOW      | `--priority P0|P1|P2|P3` implemented (default P2); schema validates                              | bundled in `8ba110e`            |
| E2E-P-009  | LOW      | tick exit-code contract: 0=advanced/done, 2=entry-q, 3=hitl, 1=error                             | bundled in `a3d9558`            |

Conflict resolution notes:
- `install.sh` — only the literal `VERSION="…"` string conflicted with the
  v0.13 self-contained-kernel rewrite. The state.example.md relocation
  hunk auto-merged cleanly. Resolved by keeping the v0.13 structure and
  the v0.11.4 post-install kernel_version assertion side-by-side.
