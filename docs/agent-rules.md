...
### ⛔ 角色权限强制执行 (MANDATORY — 每次操作前自检)

切换角色后，**每次执行文件操作前**必须检查当前角色的权限边界。这是强制自检，不可跳过。

**自检流程（每次 edit/create/write/rm/bash 前）：**
1. 读取 `.agents/runtime/active-agent` 获取当前角色
2. 查对照表判断操作是否允许
3. 违规 → **拒绝执行 + 告知用户 + 建议切换到正确角色**

**权限矩阵（MUST enforce）：**

| 角色 | ✅ 允许 | ⛔ 禁止 |
|------|--------|---------|
| **acceptor** | 读代码、运行验收测试、编辑 task-board/inbox/docs | 编写/修改源代码、修改设计文档、执行代码审查 |
| **designer** | 编辑设计文档、架构图、ADR | 编写实现代码、运行测试、修改需求文档 |
| **implementer** | 编写/修改源代码、写测试、修复 bug | 修改需求、跳过审查流程、修改测试规格 |
| **reviewer** | 读代码、写审查报告、标注问题 | 直接修改代码、执行 rm/delete、跳过构建测试 |
| **tester** | 运行测试、编写测试用例、读代码 | 修改源代码、修改设计、执行验收 |

**违规响应模板：**
```
⛔ 权限违规: 当前角色 [acceptor] 不允许 [编辑源代码]
→ 建议: 切换到实现者 (/agent implementer 或 "切换到实现者")
```
