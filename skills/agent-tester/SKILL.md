...
> ⛔ **强制输出规则**: 测试完成后，**必须**通过 `agent-fsm` 将任务状态转为 `accepting`（全部通过）或 `fixing`（发现 bug），并确保测试报告已写入 `.agents/runtime/tester/workspace/`。**未转状态 = 测试未完成。** 严禁仅跑测试而不出报告和转状态。

> 🔒 **Hook 硬约束**: `agent-pre-tool-use` hook 会**拦截** task-board.json 写入。当 `hitl.enabled: true` 时，FSM 状态转移必须有对应的 `.agents/reviews/T-NNN-tester-feedback.json` 且 `decision: "approved"`，否则写入被 **DENY**。不要尝试绕过——先完成 HITL 审批流程再转状态。
...
3. 读取设计文档 + 测试规格
4. **生成测试计划与用例** (⚠️ 在执行测试之前):
   - 输出测试计划到 `tester/workspace/test-plans/T-NNN-test-plan.md`:
     - 测试范围、策略、环境要求
     - 每个 Goal 对应的测试用例列表
     - 边界条件和异常场景
   - 生成测试用例到 `tester/workspace/test-cases/T-NNN/`
5. **HITL 审批门禁** (默认启用; 读取 `.agents/config.json` 的 `hitl.enabled`):
   - `hitl.enabled: true` → 调用 `agent-hitl-gate` skill 发布测试计划供人工审批
   - 等待审批通过后方可开始执行测试
   - 审批未通过 → 根据反馈修改 → 重新发布
   - `hitl.enabled: false` → 跳过此步骤

=== 阶段 2: 测试执行 (审批通过后) ===
6. 执行自动化测试 (Playwright/curl)
7. 如果全部通过:
