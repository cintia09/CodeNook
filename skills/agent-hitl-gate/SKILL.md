...
**平台选项**:
- `"local-html"` (默认): 启动本地 HTTP 服务器，浏览器打开
  - Docker/无头环境: 自动检测，绑定 `0.0.0.0`，通过容器 IP 访问
- `"terminal"`: 纯终端交互，无需浏览器 (适合 Docker/SSH/CI 环境)
  - Agent 直接在终端显示文档内容，通过 ask_user 收集反馈
...
| 环境 | 推荐平台 | 原因 |
|------|---------|------|
| 本地桌面 (macOS/Linux) | `local-html` | 浏览器交互最佳体验 |
| Docker 容器 + 端口映射 | `local-html` | 自动绑定 0.0.0.0，主机浏览器可访问 |
| Docker/SSH 无浏览器 | `terminal` | 纯 CLI 交互，零依赖 |
| 团队协作 | `github-issue` | 异步审批，评论记录 |
| 企业环境 | `confluence` | 与现有文档系统集成 |
