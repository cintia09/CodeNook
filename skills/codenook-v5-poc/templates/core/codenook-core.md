...
- **lock path**: no leading `/`, no `..` segment, no whitespace,
  character class `[A-Za-z0-9_./-]+` (no `:` — illegal in Windows
  filenames after the `/`→`-` slug conversion).
- **agent_id**: `[A-Za-z0-9_.@-]+` (prevents newline injection into
  YAML-ish lock files; no `:` for Windows filename safety).
...
### 21.6 Platform Support

The four helper scripts (`subtask-runner.sh`, `queue-runner.sh`,
`dispatch-audit.sh`, `hitl-adapters/terminal.sh`) are POSIX bash +
python3. Supported platforms:

| Platform | Support | Notes |
|---|---|---|
| Linux, macOS, WSL2 | ✅ First-class | Tested in CI-equivalent suite (T1, T8–T23). |
| Windows Git Bash / MSYS2 | ✅ Supported | Requires `python3` on PATH. Repo enforces LF line endings via root `.gitattributes`; clone must not rewrite to CRLF. |
| Native cmd.exe / PowerShell | ❌ Not supported | Scripts are bash. Run via Git Bash or WSL2 instead. |

`init.sh` checks `python3` on startup and exits with rc=3 (with a
helpful message pointing at Git for Windows + Python) if missing.

File paths generated by the runners (lock slugs, decision filenames,
task directories, JSONL log) use only characters valid on NTFS: no `:`,
no `*`, no `?`, no `<>|"`, no trailing `.` or space. The slug regex in
