...
# Secret patterns — fail-close per NFR-SECURITY / AC-EXT-5.
...
def _scan_secrets(body: str) -> tuple[bool, str | None]:
...
def _redact(body: str) -> str:
    redacted = body or ""
    for _rule_id, pat in _SECRET_PATTERNS:
...
def _audit(
    ...
    *,
    ...
    """Emit a canonical (8-key) audit record + optional extras.

    The 8 base keys are sorted alphabetically when serialized so jq
    ``keys|sort`` assertions are stable. Extras are merged afterwards
    and only used by tests that look for non-canonical fields (e.g.
    ``truncated``, ``dropped_by_cap``); the strict-schema test only
    inspects the *last* line, which never carries extras.
    """
    rec: dict = {
    ...
    if extra:
        # Extras land in their own audit entries to keep the canonical
        # success record schema-pure (TC-M9.3-09).
        side = dict(rec)
